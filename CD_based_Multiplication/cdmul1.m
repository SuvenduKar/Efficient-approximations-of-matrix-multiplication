function [comp,err]=cdmul1(A,B,tol)
    AB=A*B;
    [m,n]=size(A);
    ...
    while (comp*a)<=n % Check if all the components are exhausted
        ...
        err=norm(AB-((A*B_constructed)+(A_constructed*(B-B_constructed))),'fro')/norm(AB,'fro'); % This multiplication can be made optimal in cost of computation following the corresponding algorithm in the paper
        if err<tol
            return;
        end
        if (terminal+1)<=n
