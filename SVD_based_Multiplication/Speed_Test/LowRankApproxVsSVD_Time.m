...
%TO FIND Time taken for the (approximate)multiplications
% Change the A,B as per the test-case
orders=[100;500;700;1000;1500;2000;2500;3000];

tollist=[0.01]; % Tolerance list
repeat=25; % Number of times the test to be repeated
...
for k=1:length(tollist)
    ...
    % %plot graph low rank approximation mul time vs svd_time
    ...
    ylabel('Time (Second)', 'FontSize', 24);
    ...
    legend([hLeg1 hLeg2], ...
        {'Randomized Outer Product', 'SVD First Order'}, ...
    ...
    print(gcf, sprintf('Time_SymmetricToeplitz_tol_%.2f.png', tol), '-dpng', '-r600');
