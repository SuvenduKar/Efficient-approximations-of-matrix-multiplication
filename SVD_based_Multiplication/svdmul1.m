...
% This is first order SVD based approximation of A*B ( Note: this is not an
% optimized version of the code. The optimized version as discussed in the
% paper can be found under Speed_test folder)
function [comp,err]=svdmul1(A,B,tol)
    AB=A*B;
    [m,n]=size(A);
    comp=1;
    ...
    while (comp*a)<=n
        ...
        err=norm(AB-((A*B_constructed)+(A_constructed*(B-B_constructed))),'fro')/norm(AB,'fro'); % This multiplication can be implemented in an optimal way as suggested in the paper 
        if err<tol
            return;
        end
        if (terminal+1)<=n
