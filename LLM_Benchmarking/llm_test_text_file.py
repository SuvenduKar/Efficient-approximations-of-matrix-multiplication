...
=========================================================================================
*****************WE ARE USING A SAMPLE TEXT FILE HERE*****
TASK DESCRIPTION:
...
import os
##### UNCOMMENT THE BELOW TO ENSURE SINGLE_CORE RUN################
...
def timed_forward(model, inputs, n_warmup=1, n_runs=1):
    """Run n_warmup + n_runs forward passes, return median latency (s) and loss."""
    with torch.no_grad():
...
def benchmark_llm(model_id="facebook/opt-1.3b",
                  ...
                  s_values=(2, 5, 10, 20, 30, 40, 50)):
 
    device = torch.device("cpu")
    print(f"Using device: {device}")
 
    tokenizer = AutoTokenizer.from_pretrained(model_id)
    # 1. Read the required lines from the text file
    with open('paper_benchmark_sequential.txt', "r", encoding="utf-8") as f:
        # Strip newline characters to ensure clean text
        batch_lines = [line.strip() for line in f if line.strip()]

    # Ensure we only process the 25 lines (just in case)
    batch_lines = batch_lines[:25]

    # 2. Pass the entire list of 23 unique strings to the tokenizer
    inputs = tokenizer(batch_lines, return_tensors="pt",
    ...
    print(f"  Latency (median): {base_latency:.2f} s  |  Perplexity: {base_perplexity:.4f}")
    del model_baseline
    gc.collect()
    ...
    for s in s_values:
        ...
        print(f"  Latency (median): {lat:.2f} s  |  Perplexity: {ppl:.4f}")
...
def plot_results(results, base_latency, base_perplexity):
    ...
    markers = {2: 'o', 5: 's', 10: 'D',20: '^', 30:'v', 40:'P', 50:'X'}
    colors  = {2: 'purple', 5: 'teal', 10: 'gold', 20:'blue', 30:'green', 40: 'orange', 50: 'brown'}
    ...
    legend_handles = [
        ...
        mlines.Line2D([], [], color='w', marker='^',
                  ...
                  markersize=16, alpha=0.6, label='s = 20'),

        mlines.Line2D([], [], color='w', marker='v',
                    markerfacecolor='green', markeredgecolor='black',
                    markersize=16, alpha=0.6, label='s = 30'),

        mlines.Line2D([], [], color='w', marker='P',
                    markerfacecolor='orange', markeredgecolor='black',
                    markersize=16, alpha=0.6, label='s = 40'),

        mlines.Line2D([], [], color='w', marker='X',
                    markerfacecolor='brown', markeredgecolor='black',
                    markersize=16, alpha=0.6, label='s = 50'),
    ...
    plt.xlabel('End-to-End Latency (s)', fontsize=22)
    plt.ylabel('Perplexity', fontsize=22)
    plt.tick_params(axis='both', which='major', labelsize=16)
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.tight_layout()
    plt.savefig('llm_test_1.3b_text_file.png', dpi=150)
    plt.show()
 
 
if __name__ == "__main__":
    results, b_lat, b_perp = benchmark_llm(model_id="facebook/opt-1.3b")
